...
Original file is located at
    https://colab.research.google.com/drive/1gwvrTHMLh1_0HAh0EO_hX3Zghr-6Jgai
...
def printEmotionLogitsFormatted(emotionLabel: str, topK: int = 5):
    ...
    for i, entry in enumerate(resultsBottom):
        token = entry["token"]          # keep raw token
        sigmaScore = entry["zScore"]
...
#### `performBaselineEmotionProbeSteering()`
...
def performBaselineEmotionProbe(inputPrompt):
...
def plotEmotionHeatmap(heatmap, emotions):
    ...
    ax.set_title(f"{kModelIdx} Emotion Steering' ΔLogProb Ratio | Layer {gTargetLayer}")
...
steerValue = 0.5
output = performBaselineEmotionProbe(prompt)
print(f"[BASELINE] Baseline Output: {output[:200]}...")
...
for emotionLabel in emotionLabels:
    ...
    print(f"[STEERING] Output: {output[:200]}...")
